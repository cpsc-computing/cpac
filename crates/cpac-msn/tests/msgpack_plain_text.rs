...
fn msgpack_should_not_detect_plain_text() {
    ...
    assert!(!result.applied, "Plain text should not be detected as MessagePack");
    assert_eq!(result.residual.len(), plain_text.len(), "Residual should be unchanged");
...
fn msgpack_roundtrip_plain_text_forced() {
    ...
    if result.applied {
        let reconstructed = reconstruct(&result).unwrap();
        assert_eq!(reconstructed.len(), plain_text.len(), 
