...
{"name":"Eve","age":32,"city":"Boston"}"#.repeat(100);
    ...
    let mut compressor = StreamingCompressor::with_msn(config, msn_config, 512, 16 * 1024 * 1024).unwrap();
...
fn msn_streaming_csv_roundtrip() {
    ...
    let mut compressor = StreamingCompressor::with_msn(config, msn_config, 1024, 16 * 1024 * 1024).unwrap();
...
<person><name>Charlie</name><age>35</age></person>"#.repeat(100);
...
fn msn_streaming_incremental_writes() {
    ...
    // Write incrementally
    let mut compressor = StreamingCompressor::with_msn(config, msn_config, 2048, 16 * 1024 * 1024).unwrap();
    ...
    for (exp, decomp) in expected_lines.iter().zip(decompressed_lines.iter()) {
        ...
        assert_eq!(exp_json, decomp_json, "JSON objects don't match semantically");
    }
}
...
fn msn_streaming_small_blocks() {
    ...
    let mut compressor = StreamingCompressor::with_msn(config, msn_config, 256, 16 * 1024 * 1024).unwrap();
    compressor.write(json_data.as_bytes()).unwrap();
    let compressed_frame = compressor.finish().unwrap();
...
fn msn_streaming_compression_benefit() {
    ...
    let json_data = r#"{"timestamp":"2024-01-01T00:00:00Z","level":"info","message":"test","user_id":12345}
{"timestamp":"2024-01-01T00:01:00Z","level":"warn","message":"alert","user_id":67890}
{"timestamp":"2024-01-01T00:02:00Z","level":"error","message":"critical","user_id":11111}"#.repeat(50);
    ...
    let msn_config = MsnConfig::default();
    let mut compressor_msn = StreamingCompressor::with_msn(config_msn, msn_config, 512, 16 * 1024 * 1024).unwrap();
    ...
    let mut compressor_no_msn = StreamingCompressor::with_msn(config_no_msn, msn_config_disabled, 512, 16 * 1024 * 1024).unwrap();
    ...
    assert!(compressed_msn.len() < compressed_no_msn.len(), 
...
fn msn_streaming_large_data() {
    ...
    let mut compressor = StreamingCompressor::with_msn(config, msn_config, 1024 * 1024, 64 * 1024 * 1024).unwrap();
    compressor.write(json_data.as_bytes()).unwrap();
    let compressed_frame = compressor.finish().unwrap();
