...
use cpac_engine::bench::{generate_csv_export, generate_markdown_report, BenchProfile};
...
fn run_benchmark(profile: BenchProfile, profile_name: &str) {
    ...
    let summary =
        BenchmarkRunner::summarize(&format!("{}-bench", profile_name.to_lowercase()), &results);

    // Display results
    println!(
        "Total Original: {} bytes ({:.2} MB)",
        summary.total_original,
        summary.total_original as f64 / 1_048_576.0
    );
    println!(
        "Total Compressed: {} bytes ({:.2} MB)",
        summary.total_compressed,
        summary.total_compressed as f64 / 1_048_576.0
    );
